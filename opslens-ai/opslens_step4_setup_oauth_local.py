...
OAUTH_SERVICE = BACKEND_DIR / "oauth_local_service.py"

if not APP_HSMETA.exists():
    raise SystemExit(f"Could not find app-hsmeta.json at: {APP_HSMETA}")
...
auth["type"] = "oauth"
auth["redirectUrls"] = ["http://localhost:3000/oauth-callback"]

required_scopes = auth.setdefault("requiredScopes", [])
optional_scopes = auth.setdefault("optionalScopes", [])
conditionally_required_scopes = auth.setdefault("conditionallyRequiredScopes", [])

auth["requiredScopes"] = required_scopes
auth["optionalScopes"] = optional_scopes
auth["conditionallyRequiredScopes"] = conditionally_required_scopes

permitted_urls = config.setdefault("permittedUrls", {})
fetch_urls = permitted_urls.setdefault("fetch", [])
for url in ["https://api.hubapi.com"]:
...
support.setdefault("supportEmail", "support@opslens.ai")
support.setdefault("documentationUrl", "https://example.com/docs")
support.setdefault("supportUrl", "https://example.com/support")
support.setdefault("supportPhone", "+18005555555")
...
"""
OAUTH_SERVICE.write_text(textwrap.dedent(oauth_code).lstrip("\n"), encoding="utf-8")

print("OpsLens step 4 OAuth local service scaffold created successfully.")
print(f"Project root: {PROJECT_ROOT}")
print(f"Workspace root: {WORKSPACE_ROOT}")
print()
print("Updated / created files:")
print(f" - {APP_HSMETA}")
print(f" - {OAUTH_SERVICE}")
